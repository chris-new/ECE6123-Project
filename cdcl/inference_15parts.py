...
if __name__ == '__main__':
    ...
    for filename in os.listdir(args.input_folder):
        if filename.endswith(".png") or filename.endswith(".jpg"):
            ...
            # seg_canvas = pure segmentation result
            # human_seg_combine_argmax: change a segmask into rgb color
            seg_canvas = human_seg_combine_argmax(seg_argmax)
            
            np_filename = '%s/%s'%(args.output_folder,filename[:-4] +'_cdcl_seg_output.npy')
            np.save(np_filename, seg_max)
            
            filename1 = '%s/%s.jpg'%(args.output_folder,filename[:-4] +'_cdcl_segmask')
            np_filename = '%s/%s'%(args.output_folder,filename[:-4] +'_cdcl_segmask.npy')
            np.save(np_filename, seg_canvas)
            cv2.imwrite(filename1, seg_canvas) 
            
            cur_canvas = cv2.imread(args.input_folder+'/'+filename)
            canvas = cv2.addWeighted(seg_canvas, 0.6, cur_canvas, 0.4, 0)
            filename = '%s/%s.jpg'%(args.output_folder,filename[:-4] +'_cdcl_seg_blend')
